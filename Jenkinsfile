...
pipeline {
    ...
    environment {
        VERACODE_APP_NAME = 'WebGoat-dotNET-Core'      // App Name in the Veracode Platform
    }
    ...
    stages{
        ...
        stage ('Veracode scan') {
            steps {
                // zip archive for Veracode scanning.  
                zip zipFile: 'upload.zip', archive: false, glob: 'WebGoatCore/bin/Debug/net5.0/publish/**'
        ...
        stage ('Deploy') {
            ...
            steps {
                ...
                ansiColor('xterm') {
                    sh 'docker build -t webgoat-netcore:${BUILD_TAG} .'
                }
